...
def calc_struct(p,h,k,l,d,q,alphastar,astar,lattice,hh,kh,lh):
    ...
    Mlist=[M1,M2,M3,M4]
    for M in Mlist:
        ht,kt,lt=hex2twin(hh,kh,lh,M)
        hn,kn,ln=hex2rhomb(ht,kt,lt) #dangeerously assume d the same)
        cos2n1=calc_cos2n(phi1,hn,kn,ln,d,alphastar,astar)
        #cos2n2=calc_cos2n(phi2,h,k,l,d,alphastar,astar)
        s1=(1-cos2n1)
        #s2=(1-cos2n2)
        flist=N.zeros(len(h),'Float64')
        for i in range(len(h)):
            ...
            f2=f2/3
            #print 'f2inal',f2
            flist[i]=flist[i]+f2*scale**2*s1[i]
    #flist=N.array(flist)*s1*scale**2
    flist=flist/4 #twin average
...
if __name__=="__main__":
    p0=N.array([100,N.radians(60)],'Float64')
    if 0:
    ...
    if 1:
        data=N.array([[.5,-.5,1.5,55+6,5],            #192630
                     ...
                     #[2.50,-0.50,-0.50,16+2,3],  #WHY   #192661
                     #[0.50,0.50, 2.50,18+1,3], #WHY     #192632
