...
#    pc=ax.imshow(zima,interpolation='bilinear',cmap=cmap)
    #pc.set_clim(0.0,180.0)



    if colorflag:
        #g=pylab.colorbar(pc,ticks=N.arange(0,180,20))
        g=pylab.colorbar(pc)
        print g
        #g.ticks=None
...
if __name__ == '__main__':
    ...
    xdpm,ydpm,zdpm=readmeshfiles(mydirectory,'meshdzerofieldplusminus',myend) #0
    xdmp,ydmp,zdmp=readmeshfiles(mydirectory,'meshdzerofieldminusplus',myend) #0
    ...
    if 1:
        #ax,g=plot_data(xdmp,ydmp,zdmp,fig,1,colorflag=True)
        ax,g=plot_data(xdmp,ydmp,zd,fig,1,colorflag=True)
    ...
    if 0:
        ax,g=plot_data(xdpm,ydpm,zdpm,fig,3,colorflag=True)
